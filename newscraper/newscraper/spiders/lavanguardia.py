...
class LaVanguardiaSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        item = NewsItem()
        datetime_str = response.xpath("//div[@class='date-time']/time/@datetime").get()
        if datetime_str and isinstance(datetime_str, str):
            ...
            summary = response.xpath("//h2[@class='epigraph']/text()").get() or ""
